...
Функция УстановитьИмяФайлаЗапросов(стрПолноеИмя = "")
	...
	КонецЕсли;
	Если ОпцияПоказыватьПутьЗапроса Тогда
		Элементы.ПакетЗапросов.ПодчиненныеЭлементы.СписокЗапросовЗапрос.Заголовок = ПакетЗапросов_ИмяДляПодсказки;
...
&НаСервере
Функция ВернутьПолноеИмя(ТипЗначения)
	МетаданныеЗначения = Метаданные.НайтиПоТипу(ТипЗначения);
	Возврат МетаданныеЗначения.ПолноеИмя();
КонецФункции

&НаСервере
Функция ВернутьТипОбъекта(ТипЗначения)
	МетаданныеЗначения = Метаданные.НайтиПоТипу(ТипЗначения);
...
Процедура ПараметрыЗапросаЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТекущиеДанные.ТипКонтейнера = 0 Тогда
		...
		ТипОбъектаКонфигурации = ВернутьТипОбъекта(ТипЗначения);
		
		Если ТипОбъектаКонфигурации Тогда
			СтандартнаяОбработка = Ложь;
			ПолноеИмя = ВернутьПолноеИмя(ТипЗначения);
			...
			ИмяФормыРедактирования = ПолноеИмя + ".ФормаВыбора";
			ОткрытьФорму(ИмяФормыРедактирования, , ЭтаФорма, Ложь, , , ОписаниеОповещенияОЗакрытииОткрываемойФормы);
		КонецЕсли;
	...
	Если РезультатЗакрытия <> Неопределено Тогда
		...
		Если ДополнительныеПараметры.Поле = "Контейнер" Тогда
			Если ДополнительныеПараметры.Таблица = "ПараметрыЗапроса" Тогда
				ПараметрыЗапроса_СохранитьЗначение(ДополнительныеПараметры.Строка, РезультатЗакрытия);
...
&НаКлиенте
Процедура ОпцияПоказыватьПутьЗапросаПриИзменении(Элемент)
	Если ОпцияПоказыватьПутьЗапроса Тогда
		Элементы.ПакетЗапросов.ПодчиненныеЭлементы.СписокЗапросовЗапрос.Заголовок = Элементы.ПакетЗапросов.ПодчиненныеЭлементы.СписокЗапросовЗапрос.Подсказка;
	Иначе
		Элементы.ПакетЗапросов.ПодчиненныеЭлементы.СписокЗапросовЗапрос.Заголовок = ПакетЗапросов_ИмяДляОтображения; 
	КонецЕсли;
